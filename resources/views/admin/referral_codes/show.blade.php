        ...
        <div class="col-md-8">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>Code:</strong> <span class="font-monospace">{{ $referralCode->code }}</span></p>
                            <p><strong>Format:</strong> UUID-based ({{ strlen($referralCode->code) }} characters)</p>
                            <p><strong>Status:</strong>
                                <span class="badge bg-{{ $referralCode->status === 'used' ? 'success' : ($referralCode->status === 'expired' ? 'danger' : 'warning') }}">
                                    {{ ucfirst($referralCode->status) }}
                                </span>
                            </p>
                            <p><strong>Generated By:</strong>
                                @if($referralCode->generated_by)
                                    {{ \App\Models\User::find($referralCode->generated_by)->name ?? 'User Not Found' }}
                                @else
                                    N/A
                                @endif
                            </p>
                        ...
                        <div class="col-md-6">
                            <p><strong>Assigned To:</strong>
                                @if($referralCode->distributor_id)
                                    {{ \App\Models\User::find($referralCode->distributor_id)->name ?? 'User Not Found' }}
                                @else
                                    N/A
                                @endif
                            </p>
                            <p><strong>Used By:</strong>
                                @if($referralCode->used_by_user_id)
                                    {{ \App\Models\User::find($referralCode->used_by_user_id)->name ?? 'User Not Found' }}
                                @else
                                    N/A
                                @endif
                            </p>
                            <p><strong>Expires At:</strong> {{ $referralCode->expires_at ? $referralCode->expires_at->format('Y-m-d H:i') : 'N/A' }}</p>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-12">
                            <p><strong>Created At:</strong> {{ $referralCode->created_at ? $referralCode->created_at->format('Y-m-d H:i') : 'N/A' }}</p>
                            <p><strong>Updated At:</strong> {{ $referralCode->updated_at ? $referralCode->updated_at->format('Y-m-d H:i') : 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('#assign-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.referral_codes.assign", $referralCode) }}',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
